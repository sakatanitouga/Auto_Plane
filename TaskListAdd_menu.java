 ...
 * @author tomo
 */
public class TaskListAdd_menu extends Stage {
	 ...
	 * @param oya
	 */
	public TaskListAdd_menu( Stage oya ,List<TaskList> this_Tasks,int tasks_count,VBox leftbox ,ButtonUI buttonUI){
		super();
		pane = new GridPane();
		pane.setBackground(new Background(new BackgroundFill( Color.web("141414") , CornerRadii.EMPTY , Insets.EMPTY )));
		
		TaskLists = this_Tasks;
		tasklist_count = tasks_count;
		this_Box = leftbox;
	...
	private void Set_TaskListname() {
		...
		TaskListname.setStyle("-fx-background-color:"+MAIN_COLOR+";"
				+ "-fx-text-fill: WHITE");
		TaskListname.setPromptText("タスクリスト名");
	...
	private void Set_AddList() {
		Button AddList = new Button("リストの追加");
